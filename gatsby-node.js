...
exports.onCreateNode = ({ node, getNode, actions }) => {
  ...
  if (node.internal.type === `MarkdownRemark`) {
    ...
    const title = node.frontmatter.title
      ...
      : `${fileName} (no title provide in md)`;

    let shortTittle;
    if (isIndex) {
      shortTitle = node.frontmatter.shortTitle
        ? node.frontmatter.shortTitle
        : folderName;
    } else {
      shortTitle = node.frontmatter.date
  ...
  return new Promise((resolve, reject) => {
    ...
    `).then(result => {
      ...
      result.data.allMarkdownRemark.edges.forEach(({ node }) => {
        if (!node.fields) {
          //Verify that this is OK  --> IT's not I think
          //return;
        }
        ...
        if (
          node.fields &&
          node.fields.isIndex &&
          !node.fields.isSinglePageDocument
        ) {
          //template = `./src/templates/period-description-page.js`
          createPage({
            path: node.fields.slug,
            component: path.resolve(
              `./src/templates/period-description-page.js`
            ),
            context: {
        ...
        } else if (node.fields && node.fields.isSP) {
          //template = `./src/templates/period-description-page.js`
          createPage({
            path: node.fields.slug,
            component: path.resolve(
              `./src/templates/studypoint-friday-page.js`
            ),
            context: {
              // Data passed to context is available in page queries as GraphQL variables.
              slug: node.fields.slug,
        ...
        } else if (node.fields && node.fields.isSinglePageDocument) {
          createPage({
            path: node.fields.slug,
